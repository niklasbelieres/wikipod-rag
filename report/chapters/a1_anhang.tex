...
\begin{lstlisting}
  paths:
  zim_file: "data/wikipedia_en.zim"
  pageviews_file: "data/pageviews-aggregated.txt"  # produced by `wikipod fetch-pageviews` above
  data_dir: "data"

selection:
  storage_budget_mb: 13000
  weights:
    word_count: 0.15
    link_count: 0.15
    importance: 0.15
    incoming_links: 0.25
    pageviews: 0.3  # enabled here -- requires paths.pageviews_file to exist, see Usage above
...
\section{Vollständige Eval-Ergebnisse}

Die folgenden Tabellen zeigen die Einzelresultate der in
Kapitel~\ref{ch:evaluation} beschriebenen Retrieval-Evaluation. Für den
regulären Evaluationsdatensatz sind die Ergebnisse für $k=5$ dargestellt.
Da bei $k=10$ dieselben relevanten Artikel mit denselben Rangpositionen
gefunden wurden, bleiben Recall und Reciprocal Rank gegenüber $k=5$
unverändert. Lediglich die Precision sinkt aufgrund der größeren Anzahl
betrachteter Ergebnisse von $0{,}2$ auf $0{,}1$.
\small
\begin{longtable}{@{}>{\raggedright\arraybackslash}p{6.4cm}>{\raggedright\arraybackslash}p{3.6cm}p{1.5cm}p{1cm}@{}}
\caption{Einzelergebnisse des regulären Evaluationsdatensatzes bei $k=5$}
