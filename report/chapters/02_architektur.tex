...
Für die semantische Repräsentation der Textabschnitte wird die Bibliothek
sentence-\allowbreak trans\-formers verwendet. Die daraus erzeugten dichten Vektoren
werden gemeinsam mit den zugehörigen Metadaten in OpenSearch gespeichert und
dort für das $k$-NN-basierte Retrieval genutzt. KIWIX beziehungsweise ZIM als
Datenformat und OpenSearch als Retrieval-System ergeben sich unmittelbar aus
...
\begin{figure}[H]
    ...
    % Indexierungsphase
    \node[box] (zim)
        {\textbf{Wikipedia-ZIM}\\{\small Dump der Artikeldaten}};

    \node[box, below=of zim] (metadata)
        {\textbf{Metadatenextraktion}\\und JSONL-Cache};

    \node[box, below=of metadata] (selection)
        {\textbf{Link-Frequenz-Map,}\\\textbf{Scoring und Auswahl}};

    \node[box, below=of selection] (fulltext)
        {\textbf{Volltext der ausgewählten}\\\textbf{Artikel laden}};

    \node[box, below=of fulltext] (chunking)
        {\textbf{Chunking}\\{\small Text in überlappende Abschnitte}};

    \node[box, below=of chunking] (embedding)
        {\textbf{Embedding der Chunks}\\{\small Vektorrepräsentationen}};

    % Anfragephase
    \node[box, right=8.2cm of zim] (query)
        {\textbf{Nutzeranfrage}\\{\small natürliche Sprache}};

    \node[box, below=of query] (queryembed)
        {\textbf{Query-Embedding}\\{\small Vektorrepräsentation}};

    \node[box, below=of queryembed] (retrieval)
        {\textbf{Dense Retrieval in OpenSearch}\\
        {\small ähnliche Chunks suchen}};

    \node[box, below=of retrieval] (prompt)
        {\textbf{Prompt-Konstruktion}\\
        {\small relevante Chunks + Anweisung}};

    \node[box, below=of prompt] (llm)
        {\textbf{Lokales Sprachmodell}};

    \node[box, below=of llm] (answer)
        {\textbf{Antwort}\\{\small natürliche Sprache}};
...
Programmcode vornehmen zu müssen.\par
\endgroup
