...
Die Gewichte $w_s$ siind nicht fest codiert, sondern über \texttt{config/prod.yaml} beziehungsweise für den
produktiven Lauf über \texttt{config/server.yaml} konfigurierbar (Tabelle~\ref{tab:scoring-weights}).
...
\begin{table}[ht]
    ...
    \begin{tabular}{lrrrr}
        \toprule
        Signal & Gewicht $w_s$ & Mittelwert $\log(1{+}x)$ & Gewichteter Mittelwert & Anteil am Score \\
        \midrule
...
(\texttt{used\_mb}, \texttt{utilization}). Im produktiven Lauf auf dem vollen en.wikipedia-Korpus wurden von
7.322.195 Kandidatenartikeln 1.638.375 Artikel (22,4\,\%) ausgewählt. Dabei wurden 13.000,0 von 13.000~MB
Speicherbudget genutzt, was einer Budgetausschöpfung (\texttt{utilization}) von 100\,\% entspricht -- die
Greedy-Heuristik füllt das Budget damit praktisch vollständig aus.
