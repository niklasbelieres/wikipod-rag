...
Textpassagen als Retrieval-Einheiten stellt dabei einen etablierten Ansatz im Dense Retrieval dar~\cite{karpukhin-etal-2020-dense}.
...
Embedding-Modell abhängt und daher nicht allgemein festgelegt werden kann~\cite{bhat2025rethinkingchunksizelongdocument}. Die für dieses Projekt verwendete Fenstergröße wurde
...
anderem für Information-Retrieval-, Clustering- und Sentence-Similarity-Aufgaben vorgesehen~\cite{minilm-model-card}. Damit ist es für den im Projekt verfolgten Anwendungsfall des semantischen
Retrievals geeignet.

Eine weitere relevante Einschränkung des Modells ist die maximale Eingabelänge von 256 Word Pieces~\cite{minilm-model-card}. Da die Chunk-Größe anhand von Wörtern festgelegt wird, kann daraus jedoch nicht
...
\section{Retrieval}
Die Klasse \texttt{Retriever} ermöglicht es, Query-Embedding und $k$-NN-Suche zu abstrahieren. Die zentrale Methode
\texttt{retrieve(query, k)} kapselt diese Logik und gibt zu einem Anfrage-String die Top-$k$-Chunks zurück, wodurch die
OpenSearch-spezifische Antwortstruktur nach außen verborgen bleibt.

Konkret wird in \texttt{retrieve(query, k)} zunächst der Anfragestring mit derselben Embedder-Instanz wie beim Indexieren vektorisiert. Im
zweiten Schritt wird dann eine $k$-NN-Suche gegen den in~\ref{sec:opensearch-index-design} beschriebenen Index ausgeführt. Die Suche selbst liefert
dabei bereits genau $k$ Treffer, welche zur weiteren Verarbeitung in Chunk-Objekte transformiert und schließlich zurückgegeben werden.

Vorteil dieser Abstraktion ist die bereits erwähnte Kapselung des OpenSearch-spezifischen Antwortformats. Zusätzlich lässt sich durch die
\texttt{--chunks-only}-Option im \texttt{query}-Kommando das Retrieval isoliert von der LLM-Generierung ausführen bzw. testen, denn hier werden
direkt die Top-$k$-Chunks ausgegeben. Genau diese Trennbarkeit ermöglicht es, eine separate Messung der Retrieval-Qualität in Kapitel~\ref{ch:evaluation}
durchzuführen, welche isoliert von der Generierung des Sprachmodells stattfindet.